...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    val listner = createListener(relativePath)
    val listnerSubscription = listner.start
    Future.run() {
      ct => Future {
        async {
          while(ct.nonCancelled) {
            val (request, exchange) = await(listner.nextRequest)
            respond(exchange, ct, handler(request))
          }
          listnerSubscription.unsubscribe
        }
      }
    }
  }
