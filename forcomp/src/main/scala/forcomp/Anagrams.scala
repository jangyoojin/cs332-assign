...
object Anagrams {
   ...
   */
  //w.groupBy(e => e.toLower).toList.sortBy{ case (c, _) => c }.map{ case (key, chars) => (key, chars.length)}
  def wordOccurrences(w: Word): Occurrences = (for { component <- w.groupBy(e => e.toLower).toList } yield (component._1, component._2.length)).sortBy(e => e._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences.foldRight(List[Occurrences](Nil)) ((o, acc) => acc ::: (for {
      temp <- acc
      count <- 1 to o._2
